...
def updateScore(tname, addPts, solved):
	...
	cursor.execute(query)
	query = "CREATE OR REPLACE VIEW user_scores AS SELECT * FROM score ORDER BY (score) DESC, (timestmp+sub*1000) ASC"
	cursor.execute(query)
...
def Processor(threadNum):
	...
	while True:
		...
		if solved.find(prob) >= 0:
			...
			if j.language == 'java':
				replace_text = 'public class temp%d' %(threadNum)
				print 'here'
				print file_path 
				for line in fileinput.FileInput(tempfile_path,inplace=1):
					line = line.replace("public class Main",replace_text)
					print line
			...
			if j.language == 'c':
				cmd = "g++ -lm -o %s/temp%d %s/temp%d.cc > %s/compiler.txt 2>&1" % (usrpath, threadNum, usrpath, threadNum, usrpath)
			#elif j.language == 'python':    
			...
			#	cmd = "javac %s/temp%d.java > %s/compiler.txt" % (usrpath,threadNum,usrpath)    
				cmd = "javac %s/%s.java > %s/compiler.txt" % (usrpath,j.tname,usrpath)    
			ret = 0
			...
			if ret != 0 and j.language != 'python':
				...
				if j.language == 'c':
					...
					cmd = 'cd %s; java %s'%(usrpath , j.tname)
				#fd1=file("usr.txt",'rw')
				
				#p = subprocess.Popen(cmd, shell=True, stdin=fd, stdout=fd1)
				...
				while True:
					...
					# Break if subprocess has terminated or runtime has exceeded 3.0s
					if ((p.returncode != None) or (runTime > 3.00001)):
						break;
